...
const FeatureComponent: React.FC<FeatureProps> = ({ title, description, onClick }) => {
    return (
        <Box
            bg='white'
            p={6}
...
export const Features: React.FC<FeaturesProps> = ({ features, dispatch }) => (
    <VStack padding={8}>
        <Heading as="h2" size="2xl">プロジェクトの特徴</Heading>
        <SimpleGrid columns={{ base: 1, md: 2, lg: 4 }} padding={8} width="full">
            {features.map((feature, index) => (
                <FeatureComponent
                    key={index}
                    {...feature}
                    onClick={() => dispatch({ type: 'FEATURE_CLICKED', payload: feature.title })}
                />
            ))}
        </SimpleGrid>
    </VStack>
