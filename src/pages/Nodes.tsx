...
const TransactionNetwork: React.FC<TransactionNetworkProps> = ({ nodes, transactions }) => {
  ...
  const positionedNodes: PositionedNode[] = useMemo(() => placeNodesOnSphere(nodes, 10), [nodes]);

  // Create edges between all nodes
  const edges = useMemo(() => createEdgesBetweenAllNodes(positionedNodes), [positionedNodes]);
  ...
  return (
    <>
      <Canvas style={{ width: '100%', height: '100%' }} camera={{ position: [0, 0, 30] }}>
        ...
        {edges.map((edge) => (
          <EdgeComponent
            ...
            highlighted={
              (highlightedTransaction?.from === edge.from && highlightedTransaction?.to === edge.to) ||
              (highlightedTransaction?.from === edge.to && highlightedTransaction?.to === edge.from)
            }
          />
        ))}
      </Canvas>
      ...
      {highlightedTransaction && (
        <div className="transaction-details" style={transactionDetailsStyle}>
          <strong>Transaction:</strong> Node {highlightedTransaction.from} sent {highlightedTransaction.amount} to Node {highlightedTransaction.to}
        </div>
      )}
    </>
  );
...
const placeNodesOnSphere = (nodes: Node[], radius: number): PositionedNode[] => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      edges.push({
        ...
        from: nodes[i].id,
        to: nodes[j].id,
        length: new THREE.Vector3(...nodes[i].position).distanceTo(new THREE.Vector3(...nodes[j].position)),
      });
    }
  }
  return edges;
...
const EdgeComponent: React.FC<EdgeProps> = ({ edge, positionedNodes, highlighted }) => {
  ...
  return (
    <animated.mesh>
      <tubeGeometry args={[curve, 64, radius.get(), 8, false]} /> {/* Increased segments for smoothness */}
